...
int scan_cgi_header(per_request *reqInfo, int pd) 
{
   ...
   * it was:
   * if((ret = mygetline(reqInfo->cgi_buf,str,HUGE_STRING_LEN-1,0,timeout)) <= 0)
   *
   ...
   */
    ...
    while(1) {
      if((ret = mygetline(reqInfo->cgi_buf,str,HUGE_STRING_LEN-1,options,timeout)) <= 0)
      {
...
int cgi_stub(per_request *reqInfo, struct stat *finfo, int allow_options) 
{
  ...
  if(!pid) {
    ...
    if (reqInfo->inh_content_length > 0) {
      ...
      nBytes=mygetline(reqInfo->sb, szBuf,HUGE_STRING_LEN,G_FLUSH, timeout);
      nTotalBytes = nBytes;
      if (nBytes >= 0) {
  ...
  if(!nph) {
     ...
     * mygetline, but the SSI parser used buffered stdio.
     * 
...
long send_fd(per_request *reqInfo, int pd, void (*onexit)(void))
{
    ...
    if (reqInfo->cgi_buf != NULL)
      n=mygetline(reqInfo->cgi_buf, buf,IOBUFSIZE,G_FLUSH,timeout);
     else 
      n = 0;
    while (1) {
	o=0;
